...
namespace TwoBySixAntennaSwitch
{
    ...
    public class SerialUserInterface : IDisposable
    {
        ...
        public SerialUserInterface(string port = Serial.COM1)
        {
            ...
            _comPort = new SerialPort(port, 115200, Parity.None, 8, StopBits.One) { ReadTimeout = 20, WriteTimeout = 20 };
            _comPort.DataReceived += DataReceivedHandler;
            _comPort.ErrorReceived += ErrorReceivedHandler;
            _comPort.Open();
        ...
        /// <summary>
        /// Immediately send a list of strings to the serial port (appends a CRLF)
        /// </summary>
        /// <param name="list"></param>
        public void DisplayLine(ArrayList list)
        {
            foreach (string line in list)
            {
                DisplayLine(line);
        ...
        /// <summary>
        /// Immediately send a line of text to the serial port (appends a CRLF)
        /// </summary>
        /// <param name="line"></param>
        ...
        public void Go()
        {
            ...
            {
                inputList.Add(item.Option.PadLeft(8) + " " + item.Label + "\r\n");
            }
